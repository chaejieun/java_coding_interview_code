package main.java.RotateMatrix.coding.challenge;
...
public final class Arrays {
    ...
    public static int[][] rotateInNew(int[][] m) {
        ...
        for (int i = 0; i < m.length; i++) {
            for (int j = 0; j < m.length; j++) {
                r[offset - j][i] = m[i][j];
                String t = Integer.toString(offset-j);
                System.out.println("offset - j, i : " +  t + " , " + i);
            }
        }
    ...
    private static void transpose(int m[][]) {
    	System.out.println("m[0].lenght: " + m[0].length );
